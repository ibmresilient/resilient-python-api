#!/bin/bash -e

function getJsonVal() {
   releaseID=`python -c "import json,sys;print json.dumps(json.loads('''$1''')$2); sys.exit(0);"` ; 
   echo $releaseID
}

echo Getting Git Tag...
latestTag=$(git describe --tags)

echo Creating release...
echo '{"tag_name": "'$latestTag'","target_commitish": "master","name": "Internal Release '$latestTag'","body": "Generated by Jenkins","draft": true,"prerelease": true}' > json.json
output=$(curl -X POST -H 'Content-Type:application/json' -H 'Accept:application/json' --data-binary @json.json https://github.ibm.com/api/v3/repos/Resilient/resilient-python-api/releases?access_token=e559c907d124e572df9f18b7421b3d94847e39b3)
rm json.json
id=$(getJsonVal "$output" "['id']")
echo Release Id: $id
for file in ../builds/resilient*.tar.gz; do
  echo Uploading file... ${file##*/}
  fileUpload=$(curl --data-binary @../builds/${file##*/} -H "Authorization: token e559c907d124e572df9f18b7421b3d94847e39b3" -H "Content-Type: application/octet-stream" https://github.ibm.com/api/uploads/repos/Resilient/resilient-python-api/releases/"$id"/assets?name=${file##*/} )
  echo File uploaded
done






