...
class TestMakeRestCall(unittest.TestCase):
    ...
    # TODO: should this go into its own file?
    class MockResponse:
    ...
    @parameterized.expand(REQUESTS_COMMON_CLASSES)
    def test_json_request(self, RCObjectType):
        RequestsCommon.execute = self.MockRC.execute
        rc = RCObjectType()
        response = rc.make_rest_call(
            method=self.method,
            url=self.url,
    ...
    @parameterized.expand([[RequestsCommon, {"content-type" : "application/json"}], [RequestsCommonWithoutSession, {"content-type" : "application/json"}],
                           [RequestsCommon, {"CONTENT-TYPE" : "application/json"}], [RequestsCommonWithoutSession, {"CONTENT-TYPE" : "application/json"}],
                           [RequestsCommon, {"Content-Type" : "application/json"}], [RequestsCommonWithoutSession, {"Content-Type" : "application/json"}],
                           [RequestsCommon, {"CoNtEnt-TyPe" : "application/json"}], [RequestsCommonWithoutSession, {"CoNtEnt-TyPe" : "application/json"}],
                           [RequestsCommon, {"ctype" : "application/json"}], [RequestsCommonWithoutSession, {"ctype" : "application/json"}]])
    def test_json_lowercase_content_type(self, reqc, header):
        RequestsCommon.execute = self.MockRC.execute
        rc = reqc()
        response = rc.make_rest_call(
            method=self.method,
        ...
        assert response["url"] == self.url
        assert response["method"] == self.method
        self.assertDictEqual(response["headers"], header)
        self.assertDictEqual(response["json"], {"key1" : "body1"})
        self.assertIsNone(response["data"])
