    ...
    if tox_args:
        ...
        for arg in tox_args:
            ...
            if not match or len(match.groups()) != 2:
                LOG.warning("WARNING: skipping argument '{0}' given in --tox-args flag that doesn't match format attr=\"value\"".format(arg))
                continue
        ...
        # is parsed above
        LOG.warning("Using mock args: {0}. -h for more info".format(constants.TOX_TESTS_DEFAULT_ARGS))
        args.extend(constants.TOX_TESTS_DEFAULT_ARGS)
...
def _tox_tests_parse_xml_report(path_xml_file):
    ...
    if root.tag == "testsuites":
        ...
        LOG.warning("WARNING: XML report generated by tox run was not readable. Consider upgrading tox and pytest to the latest versions")
        return -1, -1, -1, "", ""
