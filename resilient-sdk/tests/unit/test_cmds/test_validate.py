...
def test_pass_validate_setup_py_file():

    with patch("resilient_sdk.cmds.validate.sdk_helpers.validate_file_paths") as mock_file_valid:
        mock_file_valid.return_value = None

        # this only works because validate_file_paths is patched to None so that the filepath isn't validated
        mock_package_path = "fake/path/that/isnt/used"
...
def test_fail_validate_setup_py_file():
...
def test_pass_validate_selftest_py_file(fx_copy_fn_main_mock_integration):
    ...
    with patch("resilient_sdk.cmds.validate.sdk_helpers.validate_file_paths") as mock_file_valid:
        ...
        with patch("resilient_sdk.cmds.validate.validation_configurations.selftest_attributes", new=mock_data):
            with patch("resilient_sdk.cmds.validate.package_helpers.parse_setup_py") as parse_mock:
                parse_mock.return_value = {"name": fx_copy_fn_main_mock_integration[0]}
...
def test_fail_validate_selftest_py_file(fx_copy_fn_main_mock_integration):
    ...
    with patch("resilient_sdk.cmds.validate.sdk_helpers.validate_file_paths", side_effect=None):
        mock_package_path = "fake/path/that/isnt/used"
        mock_data = [
            { "func": lambda **x: (True, SDKValidateIssue("pass", "pass", SDKValidateIssue.SEVERITY_LEVEL_DEBUG)) },
